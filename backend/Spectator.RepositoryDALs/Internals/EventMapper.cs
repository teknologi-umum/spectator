...
namespace Spectator.RepositoryDALs.Internals {
	internal record EventMapper(
		Type Type,
		string FluxTypeName,
		ConstructorInfo Constructor,
		ImmutableList<FluxPropertyInfo> FluxProperties
	) {
		public static EventMapper For<T>() {
			if (typeof(T) is { IsClass: false } or { IsAbstract: true }) {
				throw new InvalidOperationException("Event type must be a concrete class");
			}

			if (!typeof(EventBase).IsAssignableFrom(typeof(T))) {
				throw new InvalidOperationException("Event type must be derived from EventBase");
			}
			...
			return new(
				...
				FluxProperties: typeof(T)
					...
					.Where(prop => prop.Name is not nameof(EventBase.SessionId) and not nameof(EventBase.Timestamp))
		...
		public PointData ConvertToPointData<T>(T @event) where T : EventBase {
			if (typeof(T) != Type) {
