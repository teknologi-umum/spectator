...
def main():
    ...
    for user in users:
        ...
        event = generate_session_started_event(current_session, time)
        ...
        event = generate_personal_info_submitted_event(user, time)
        current_session_events.append(event)
        ...
        event = generate_before_exam_SAM_submited_event(current_session, time)
        current_session_events.append(event)
        ...
        event = generate_exam_started_event(current_session, time)
        current_session_events.append(event)
        ...
        for i in range(0, 10):
            ...
            if random_int == 0:
                event = generate_exam_ide_reloaded_event(current_session, random_time)
                current_session_events.append(event)
            elif random_int == 1:
        ...
        for _ in range(random.randint(50_000, 60_000)):
        ...
        if random_int == 0:
            ...
            event = generate_deadline_passed_event(current_session, time)
            current_session_events.append(event)
        elif random_int == 1:
            ...
            event = generate_exam_ended_event(current_session, time)
            current_session_events.append(event)
        elif random_int == 2:
            ...
            event = generate_exam_forfeited_event(current_session, time)
            current_session_events.append(event)

        # finally, they will submit a SAM test after the exam
        # let's assume every sam test requires at least 5 minutes and at most 15 minutes
        time = time + random.randint(5, 15) * MINUTES_TO_MILLIS
        event = generate_after_exam_SAM_submited_event(current_session, time)
        current_session_events.append(event)
